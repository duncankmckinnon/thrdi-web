...
description: Mirror completed Claude Code and Codex turns as nested OpenTelemetry traces.
order: 3
---

thirdeye is local-first, but it can optionally mirror each captured Claude Code or Codex session to [Pydantic Logfire](https://pydantic.dev/logfire). Export is live once enabled; there is no separate batch-export step.

## Enable export

Install the optional dependency, then save the Logfire gateway key for your project:

```bash
pip install 'thrdi[logfire]'
thirdeye logfire enable --token pylf_v1_... --project my-project
thirdeye logfire status
```

You can also enable it from **Settings** in `thirdeye ui`. Both paths persist the same settings in `~/.thirdeye/config.yaml`. The project name is a local label for your reference; the gateway key selects the Logfire destination.

To stop exporting while keeping the saved key and project:
...
Each thirdeye session becomes one Logfire trace, searchable by `gen_ai.conversation.id` and separated by `claude` or `codex` service name.
...
Claude Code can produce multiple model-call spans within one turn. Codex reconstructs its model calls and tool children from the completed rollout, deduplicating cumulative token reports before calculating usage.
...
Enabling Logfire sends captured prompts, responses, tool inputs/results, and span attributes to the configured Logfire project. Leave it disabled if traces must remain exclusively local.
...
- **`package installed : False`.** Install the extra with `pip install 'thrdi[logfire]'` in the same environment that provides `thirdeye`.
