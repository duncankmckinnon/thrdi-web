...
> Local-first observability CLI for Claude Code, Codex, Cursor, and GitHub Copilot CLI, with optional live export to Pydantic Logfire.

thirdeye (\`thrdi\` on PyPI, \`thirdeye\` on Homebrew) captures Claude Code, Codex, Cursor, and GitHub Copilot CLI sessions into a unified, searchable history stored at \`~/.thirdeye/\`. Local storage is the default. The browser UI and Pydantic Logfire export ship with every installation — no optional extras to install. Logfire export mirrors completed turns as OpenTelemetry traces with agent-turn, model-call, tool-call, permission-request, and nested-subagent spans. Per-turn token usage is captured to JSONL plus a SQLite index, and evals can be dispatched as LLM-as-judge rubrics against recorded sessions.
