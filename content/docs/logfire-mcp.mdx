...
description: Let Claude Code, Codex, Cursor, or Copilot investigate thirdeye traces using Logfire's hosted MCP server.
order: 1
parent: logfire
---
...
Replace the US URL in these examples when the project is in another region. See Pydantic's [Logfire MCP setup guide](https://pydantic.dev/docs/logfire/guides/mcp-server/) for additional clients, including GitHub Copilot CLI, and non-browser authentication.
...
```text
In the agent-tracing Logfire project, compare Claude Code, Codex, Cursor, and
Copilot over the last seven days. Show completed and interrupted turns, p50/p95 turn
...
The MCP server queries Logfire's `records` table with Apache DataFusion SQL. thirdeye uses `service_name` for the platform (`claude`, `codex`, `cursor`, or `copilot`) and stores session, token, cost, tool, and turn-status data in span attributes.
