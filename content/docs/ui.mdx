...
The Homebrew package includes the browser UI and all of its dependencies:

```bash
brew install duncanmckinnon/tap/thirdeye
```

For Python installations, the UI ships as an optional extra so the default `pip install thrdi` stays lean:
...
thirdeye serve                     # full alias of `thirdeye ui`, same flags
```

The CLI opens your default browser unless `--no-browser` is passed. `Ctrl-C` stops the server. `thirdeye serve` launches the same server with the same host, port, and browser flags — use whichever name you prefer.
...
- **`ModuleNotFoundError: starlette`** — a Python installation is missing the `ui` extra. Run `pip install 'thrdi[ui]'` (or `pipx inject thrdi starlette uvicorn jinja2`). Homebrew installs already include it.
