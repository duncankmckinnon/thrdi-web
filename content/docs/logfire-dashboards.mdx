...
The same structure works for Cursor and Copilot by including `cursor` and `copilot` in each platform filter.
...
the last seven days. Include Claude Code, Codex, Cursor, and Copilot. Organize panels
...
WHERE span_name IN ('agent-turn', 'agent_turn')
  AND service_name IN ('claude', 'codex', 'cursor', 'copilot')
GROUP BY x, series
ORDER BY x
LIMIT 500
```
...
FROM records
WHERE service_name IN ('claude', 'codex', 'cursor', 'copilot')
