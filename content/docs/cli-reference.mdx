...
| `thirdeye ui` / `thirdeye serve` | Launch the local browser UI (see [Browser UI](/docs/ui)). |
...
Walks through tracing setup for Claude Code, Codex, and Cursor; installation of bundled agent skills; and optional live export to Pydantic Logfire. Choosing Logfire signs you in through the browser and mints the write token for you — there is no key to paste.
...
`--list` prints the currently supported platforms and warns about stale hooks from removed platforms. Cursor installs hooks for both its IDE and CLI in `~/.cursor/hooks.json`. For Codex, `--force` replaces an existing `notify` program.
...
Launch the local browser UI. Defaults to `http://127.0.0.1:8765` and auto-opens your default browser. `thirdeye serve` is a full alias of `thirdeye ui` — same host, port, and browser flags. Python installs require the `ui` extra (`pip install 'thrdi[ui]'`); Homebrew includes it.
