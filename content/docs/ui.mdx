...
The browser UI ships with every Python installation too — no extra to opt into:

```bash
pip install thrdi
```

Using `pipx`? Install thirdeye normally:
...
- **`ModuleNotFoundError: starlette`** — the installation is incomplete. Reinstall with `pip install --upgrade --force-reinstall thrdi`.
