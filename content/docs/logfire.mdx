...
# Logfire write token (gateway key): [hidden input]
...
`thirdeye logfire enable` securely prompts for the token; it does not accept the token as a command-line option, so the value is not exposed in shell history. You can also enable it as part of the interactive `thirdeye setup` wizard or from **Settings** in `thirdeye ui`. All three paths persist the same settings in `~/.thirdeye/config.yaml`. The gateway key selects the Logfire destination.
