...
[workbench](https://wbcli.com) dispatches coding agents through implementation, testing, review, fix, and wrap-up stages. Each invocation becomes a separate Claude Code, Codex, Cursor, or Copilot session. thirdeye can tag those sessions automatically so the complete run remains easy to query as one plan.
...
thirdeye currently captures workbench roles assigned to Claude Code, Codex, Cursor, or Copilot. Roles assigned to another harness will continue to run, but their sessions will not appear in thirdeye.
...
- **No sessions appear.** Register the harness with `thirdeye add --claude`, `thirdeye add --codex`, `thirdeye add --cursor`, or `thirdeye add --copilot`, then start a new workbench run.
- **Only some roles appear.** Inspect the effective workbench profile. thirdeye captures Claude Code, Codex, Cursor, and Copilot, so roles assigned to other harnesses are not recorded.
