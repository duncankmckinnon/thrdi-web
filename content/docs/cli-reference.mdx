...
Walks through tracing setup for Claude Code, Codex, and Cursor; installation of bundled agent skills; and optional live export to Pydantic Logfire. The Logfire write token is collected through a hidden prompt.
...
Securely prompts for the Logfire write token, persists it, and enables live export. The token is not accepted as a command-line option. Python installs require `pip install 'thrdi[logfire]'`; Homebrew includes Logfire support.
